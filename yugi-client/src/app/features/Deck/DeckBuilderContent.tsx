...
const DeckBuilderContent: React.FC = () => {
    ...
    const fetchDeck = async () => {
      try {
        ...
        const filteredCards = allCards.filter(
          (card) => !userDeck.some((d: CardItem) => d.id === card.id)
        );
  ...
  // --- Funkcija za učitavanje dostupnih karata sa search/filter ---
  ...
  const handleRemoveCard = async (card: CardItem) => {
    setDeck((prev) =>
      prev.map((c) => (c?.id === card.id ? null : c))
    );
